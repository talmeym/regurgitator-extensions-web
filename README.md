...
as well as being able to run regurgitator in tomcat, you can also use [``regurgitator-extensions-jetty``](https://github.com/talmeym/regurgitator-extensions-jetty?tab=readme-ov-file#regurgitator-extensions-jetty){:target="_blank"} to run both ``RegurgitatorServlet`` and ``GlobalMetadataServlet`` as a standalone app, for packaging as a dockerized deployable. instructions on how this can be done are within the readme files of our reference projects [here](https://talmeym.github.io/regurgitator-all#reference-projects)
