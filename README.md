...
as well as being able to run regurgitator in tomcat, you can also use [``regurgitator-extensions-jetty``](https://talmeym.github.io/regurgitator-extensions-jetty/#regurgitator-extensions-jetty) to run both ``RegurgitatorServlet`` and ``GlobalMetadataServlet`` as a standalone app, ready for packaging as a containerized deployable. instructions on how this can be done with docker can be found in the readme files of our reference projects [here](https://talmeym.github.io/regurgitator-all#reference-projects)
